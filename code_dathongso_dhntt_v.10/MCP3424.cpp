...
ConvStatus MCP3424::nb_readf(Channel ch, double & value) {
    ...
    value = 000.001 * lval / (1 << (cread.srate << 1)) / ( 1 << cread.pga);
...
ConvStatus MCP3424::nb_read(Channel ch, long & value) {
    ...
    value = lval; //000.001 * lval / (1 << (cread.srate << 1)) / ( 1 << cread.pga);
